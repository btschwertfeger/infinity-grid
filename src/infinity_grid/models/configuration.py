...
class NotificationConfigDTO(BaseModel):
    ...
    @classmethod
    def validate_status_update_interval(cls, value: int) -> int:
        """Validate status_update_interval is positive."""
        if value <= 0:
            raise ValueError("status_update_interval must be greater than 0")
        return value
